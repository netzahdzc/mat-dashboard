...
while($row=$db->fetch_array($consulta)){
    ...
    if($row["status"]=="testCompleted"){
        ...
        echo "<td align='center'><font color='#".$fontcolor."'><a target='_blank' href='./eventsDetector.php?patient_id=".$row["patient_id"]."&test_id=".$row["id"]."&type_test=".$row["type_test"]."&test_option=".$row["test_option"].$duration."'>Open</a></font></td>";
        ...
        if($row["type_test"]=="1" || $row["type_test"]=="2"){
            ...
            echo "<a target='_blank' href='./print_data.htm?fname=post_cleaning_".$row["patient_id"]."@T".$row["id"]."Acc&type=".getTestTypeId($row["type_test"])."&dir=acc&sensibility=".getSensibility($row["type_test"],$sensibility)."'>post</a></font></td>";
        }else{
